...
async def aio_s3_bucket_head(
    bucket_name: str, config: AioAWSConfig, s3_client: aiobotocore.client.AioBaseClient
) -> Dict:
    """
        ...
        try:
            ...
            if err_code == "TooManyRequestsException":
                if tries < config.retries:
                    await jitter("s3-bucket-poke", config.min_jitter, config.max_jitter)
...
async def aio_s3_buckets_list(
    config: AioAWSConfig, s3_client: aiobotocore.client.AioBaseClient
) -> Dict:
    """
...
async def aio_s3_object_head(
    s3_uri: str, config: AioAWSConfig, s3_client: aiobotocore.client.AioBaseClient
) -> Dict:
    """
